...
if($check<1){
  echo "<div align='center' style='color:#DC533F; text-shadow:2px 2px 2px black; margin-top:100px;'><h2>Belum ada satupun artikel disini</h2></div>";}
   ...
   $ambil_artikel=("SELECT judul_berita_news,sinopsis_berita_news,isi_berita_news,foto_news,kategori,penerbit,link_news,tanggal_post FROM artikel_news where kategori ='Corat-Coret Mahasiswa' order by kategori");
	$eksekusi=mysql_query($ambil_artikel);
	 //$tampil_artikel  = $ambil_artikel->fetch(PDO::FETCH_ASSOC);
	while($tampil_artikel=mysql_fetch_array($eksekusi))
	{
  ...
  $tanggal_post=$tampil_artikel['tanggal_post'];
...
<div style="color:#0000ff; margin-bottom:10px; background-color:#FCFCFC; border-radius:12px; border:#333333 3px inset;">
	...
	<!-- head atas-->
     ...
     <div style="float:right; color:#838383; font-size:1.55em; font-family:'century Gothic'; margin-top:10px; margin-right:25px;">E - Magazine Birama<p style="font-family:'century Gothic'; font-size:11px; color:#CCCCCC;"><font color="#FF3300" style="font-family:'century Gothic';">Bacaan Interaktif Aktualisasi Mahasiswa</font></p>
        ...
        <p style="padding-top:5px; border-radius:10px; color:#009F55;; font-size:1.75em;; font-family:'century Gothic';"><?php echo $Kategori; ?>           </p>
...
<tr>
  ...
  <tr>
    ...
    <p style="float:right;" > <a href="../try_e_mag/full_global_article_corat_coret.php?page=full_global_artikel_corat_coret&amp;id_news=<?php echo $id_news."&judul_berita_news=".$Judul_Berita_News; ?>" target="_blank"><img src="icons/readmore.png" height="50" width="75"/></a>
